...
/*   Updated: 2023/03/08 20:42:19 by mgranate_ls      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_ray_hit(t_cub *cub, t_dist *dist)
{
	...
	while (hit == 0)
	{
		...
		|| mtx[dist->mapX][dist->mapY] == '2' \
		|| mtx[dist->mapX][dist->mapY] == '3')
		{
			hit = 1;
...
void	calc_sidedist(t_cub *cub, t_dist *dist)
{
	...
	{
		dist->stepX = 1;
		dist->sideDistX = (dist->mapX + 1 - cub->player.posX) \
		* dist->deltaDistX;
	}
	if (dist->raydirX < 0)
	{
	...
	{
		dist->stepY = 1;
		dist->sideDistY = (dist->mapY + 1 - cub->player.posY) \
		* dist->deltaDistY;
	}
}
